...
describe('Paste Lifecycle Integration Tests', () => {
  ...
  describe('View Limit Functionality', () => {
    it('should prevent access after max_views is reached', async () => {
      ...
      const firstGetResponse = await GET(firstGetRequest, { params: { id: pasteId } });
      const firstGetData = await getJsonResponse(firstGetResponse);
      ...
      const secondGetResponse = await GET(secondGetRequest, { params: { id: pasteId } });
      const secondGetData = await getJsonResponse(secondGetResponse);

      expect(secondGetResponse.status).toBe(404);
      expect(secondGetData.error).toBe('Paste not found');
    });
    ...
    it('should track remaining views correctly with multiple views', async () => {
      ...
      for (let viewCount = 1; viewCount <= 3; viewCount++) {
        ...
        const getResponse = await GET(getRequest, { params: { id: pasteId } });
  ...
  describe('TTL Expiry Functionality', () => {
    it('should prevent access after TTL expires', async () => {
      ...
      const beforeExpiryResponse = await GET(beforeExpiryRequest, { params: { id: pasteId } });
      const beforeExpiryData = await getJsonResponse(beforeExpiryResponse);
      ...
      const afterExpiryResponse = await GET(afterExpiryRequest, { params: { id: pasteId } });
      const afterExpiryData = await getJsonResponse(afterExpiryResponse);
  ...
  describe('Combined Constraints', () => {
    it('should expire when TTL is reached even if views remain', async () => {
      ...
      const expiredResponse = await GET(expiredRequest, { params: { id: pasteId } });
      const expiredData = await getJsonResponse(expiredResponse);

      expect(expiredResponse.status).toBe(404);
      expect(expiredData.error).toBe('Paste not found');
    });
    ...
    it('should expire when view limit is reached even if TTL remains', async () => {
      ...
      const firstResponse = await GET(firstRequest, { params: { id: pasteId } });
      ...
      const secondResponse = await GET(secondRequest, { params: { id: pasteId } });
      const secondData = await getJsonResponse(secondResponse);

      expect(secondResponse.status).toBe(404);
      expect(secondData.error).toBe('Paste not found');
    });
  });
